...
- **Desktop:** `mousemove` updates NDC mouse; click on sun collider starts transition; hover raycast drives tooltip + cursor.
- **Mobile:** `touchstart` (passive) raycasts planets + sun; tap planet locks tooltip; tap empty space clears; tap sun transitions.
